...
namespace EX {
	...
	public static class MathEX {
		...
		public static Vector2 Add(this Vector2 vector, Vector2 add) => new Vector2(vector.x + add.x, vector.y + add.y);

        public static Vector2 Vector2AreaClamp(Vector2 vector, Vector2 min, Vector2 max)
        {
        ...
        public static Vector3 Vector2AreaClamp(Vector3 vector, Vector2 min, Vector2 max)
        {
            vector.x = Mathf.Clamp(vector.x, min.x, max.x);
            vector.y = Mathf.Clamp(vector.y, min.y, max.y);
		...
		//public static AnimationCurve GenerateCurveFromEasing(Ease easing, float steps = 100f)
		//{
		//	AnimationCurve curve = new AnimationCurve();
            
		//	for (float t = 0; t <= 1; t+=(1/steps))
		//	{
  //              curve.AddKey(t, DOVirtual.EasedValue(0, 1, t, easing));
  //          }
